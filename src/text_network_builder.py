...
def build_unweighted_graph(nodes, adjacency_counts):
    ...
    G.add_nodes_from(nodes)  # Replace this line
...
def compute_distance_matrix(nodes, adjacency_counts, distance_mode="inverted"):
    ...
    for (a, b), count in adjacency_counts.items():  # Replace [] with adjacency_counts.items()
        if a in node_index and b in node_index:  # Replace False with condition: a in node_index and b in node_inde
...
def replace_rare_tokens(tokens, rare_threshold=0.01, rare_token="<RARE>"):
    ...
    rare_set = set({word for word, count in word_counts.items() if count / total_words < rare_threshold})  # Replace this line
...
def get_spatial_adjacencies(quantized_image):
    ...
    print(f"Image dimensions: {height}h x {width}w x {channels}c")  # Debug helper (optional)
...
class TestUnifiedNetworks(unittest.TestCase):
    ...
    def test_build_unweighted_graph(self):
        ...
        self.assertIsInstance(G, nx.Graph, "G should be a NetworkX Graph instance")
        ...
        self.assertEqual(G.number_of_nodes(), len(nodes), "Graph should have correct number of nodes")
        ...
        self.assertEqual(G.number_of_edges(), len(adjacency_counts), "Graph should have correct number of edges")
        ...
        for node1, node2 in adjacency_counts.keys():
            self.assertTrue(G.has_edge(node1, node2), f"Graph should have edge between {node1} and {node2}")
        ...
        self.assertEqual(set(G.nodes()), set(nodes), "Graph should contain all expected nodes")
    ...
    def test_compute_distance_matrix_inverted_mode(self):
        ...
        expected_count_matrix = np.array([[0, 3], [3, 0]])
        np.testing.assert_array_almost_equal(count_matrix, expected_count_matrix, decimal=1, err_msg="Count matrix not symmetrized correctly")
        ...
        expected_distance_matrix = np.array([[0, 1], [1, 0]])
        np.testing.assert_array_almost_equal(distance_matrix, expected_distance_matrix, decimal=1 , err_msg="Distance matrix not computed correctly in inverted mode")
        ...
        np.testing.assert_array_equal(np.diag(distance_matrix), [0, 0], err_msg="Distance matrix diagonal is not zero")
        np.testing.assert_array_equal(np.diag(count_matrix), [0, 0], err_msg="Count matrix diagonal is not zero")
    ...
    def test_tokenize_text_basic(self):
        ...
        expected1 = ['hello', ',', 'world', 'how', 'are', 'you']
        self.assertEqual(tokens1, expected1, "Basic sentence with punctuation")
        
        # TEST CASE 2: Numbers should be removed/ignored
        text2 = "I have 3 cats and 2 dogs."
        tokens2 = tokenize_text(text2)
        expected2 = ['i', 'have', 'cats', 'and', 'dogs', '.']
        self.assertEqual(tokens2, expected2, "Numbers should be removed")
        
        # TEST CASE 3: Mixed numbers within words
        text3 = "abc123def 456 ghi789"
        tokens3 = tokenize_text(text3)
        expected3 = ['abc', 'def', 'ghi']
        self.assertEqual(tokens3, expected3, "Numbers within words should be removed")
        
        # TEST CASE 4: Apostrophes should be preserved (by default)
        text4 = "Don't can't won't it's"
        tokens4 = tokenize_text(text4, char_like=["'"])
        expected4 = ["don't", "can't", "won't", "it's"]
        self.assertEqual(tokens4, expected4, "Apostrophes should be preserved")
        
        # TEST CASE 5: Only commas and periods kept as punctuation tokens
        text5 = "Hi! There? Yes, okay."
        tokens5 = tokenize_text(text5)
        expected5 = ['hi', 'there', 'yes', ',', 'okay', '.']
        self.assertEqual(tokens5, expected5, "Only commas and periods should be kept")
        
        # TEST CASE 6: Multiple commas and periods
        text6 = "First, second, third. Done."
        tokens6 = tokenize_text(text6)
        expected6 = ['first', ',', 'second', ',', 'third', '.', 'done', '.']
        self.assertEqual(tokens6, expected6, "Multiple punctuation marks")
        
        # TEST CASE 7: All uppercase should become lowercase
        text7 = "HELLO WORLD"
        tokens7 = tokenize_text(text7)
        expected7 = ['hello', 'world']
        self.assertEqual(tokens7, expected7, "Uppercase to lowercase conversion")
        
        # TEST CASE 8: Special symbols should be ignored
        text8 = "email@example.com costs $50 #hashtag"
        tokens8 = tokenize_text(text8)
        expected8 = ['email', 'example', '.', 'com', 'costs', 'hashtag']
        self.assertEqual(tokens8, expected8, "Special symbols should be removed")
        
        # TEST CASE 9: Multiple spaces and tabs
        text9 = "hello    world\t\tfoo"
        tokens9 = tokenize_text(text9)
        expected9 = ['hello', 'world', 'foo']
        self.assertEqual(tokens9, expected9, "Multiple spaces handling")
        
        # TEST CASE 10: Empty string
        text10 = ""
        tokens10 = tokenize_text(text10)
        expected10 = []
        self.assertEqual(tokens10, expected10, "Empty string should return empty list")
        
        # TEST CASE 11: Only punctuation and numbers
        text11 = "123, 456. 789!"
        tokens11 = tokenize_text(text11)
        expected11 = [',', '.']
        self.assertEqual(tokens11, expected11, "Only punctuation should remain")
        
        # TEST CASE 12: Sentence with period at end
        text12 = "The cat sat on the mat."
        tokens12 = tokenize_text(text12)
        expected12 = ['the', 'cat', 'sat', 'on', 'the', 'mat', '.']
        self.assertEqual(tokens12, expected12, "Normal sentence with period")
        
        # TEST CASE 13: Without apostrophe preservation
        text13 = "don't"
        tokens13 = tokenize_text(text13, char_like=[])
        expected13 = ['don', 't']
        self.assertEqual(tokens13, expected13, "Apostrophe as separator when not in char_like")
        
        # TEST CASE 14: Unicode/accented characters (should work with isalpha())
        text14 = "cafe naive"
        tokens14 = tokenize_text(text14)
        expected14 = ['cafe', 'naive']
        self.assertEqual(tokens14, expected14, "Unicode characters should be preserved")
        
        # TEST CASE 15: Consecutive punctuation
        text15 = "Hello,, world.."
        tokens15 = tokenize_text(text15)
        expected15 = ['hello', ',', ',', 'world', '.', '.']
        self.assertEqual(tokens15, expected15, "Consecutive punctuation handling")
        
        print("All tokenization test cases passed!")
    ...
    def test_get_spatial_adjacencies_basic(self):
        ...
        self.assertEqual(len(unique_colors), 4, "Should find exactly 4 unique colors")
        ...
        self.assertEqual(set(color_frequencies.values()), {1}, 
                     "Each color should appear exactly once in a 2x2 image")
        ...
        self.assertEqual(total_adjacencies, 8, 
                     "2x2 grid with all different colors should have 8 directional adjacencies")
        ...
        self.assertGreater(adjacency_counts[((0, 0, 0), (1, 1, 1))], 0,
                   "Top-left should be adjacent to top-right")
        self.assertGreater(adjacency_counts[((0, 0, 0), (2, 2, 2))], 0,
                        "Top-left should be adjacent to bottom-left")

        # (1,1,1) at top-right should be adjacent to:
        #   - (0,0,0) to the left
        #   - (3,3,3) below
        self.assertGreater(adjacency_counts[((1, 1, 1), (0, 0, 0))], 0,
                        "Top-right should be adjacent to top-left")
        self.assertGreater(adjacency_counts[((1, 1, 1), (3, 3, 3))], 0,
                        "Top-right should be adjacent to bottom-right")

        # (2,2,2) at bottom-left should be adjacent to:
        #   - (0,0,0) above
        #   - (3,3,3) to the right
        self.assertGreater(adjacency_counts[((2, 2, 2), (0, 0, 0))], 0,
                        "Bottom-left should be adjacent to top-left")
        self.assertGreater(adjacency_counts[((2, 2, 2), (3, 3, 3))], 0,
                        "Bottom-left should be adjacent to bottom-right")

        # (3,3,3) at bottom-right should be adjacent to:
        #   - (1,1,1) above
        #   - (2,2,2) to the left
        self.assertGreater(adjacency_counts[((3, 3, 3), (1, 1, 1))], 0,
                        "Bottom-right should be adjacent to top-right")
        self.assertGreater(adjacency_counts[((3, 3, 3), (2, 2, 2))], 0,
                        "Bottom-right should be adjacent to bottom-left")
